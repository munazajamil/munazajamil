<h1 align="center">Hi, I'm Munnaza 👋</h1>
<h3 align="center">Aspiring SOC Analyst | Blue Team | SIEM • Log Analysis • Threat Detection</h3>
<p align="center">
  <a href="https://www.linkedin.com/in/munaza-jamil1/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="https://tryhackme.com/p/munaza.jamil01"><img src="https://img.shields.io/badge/TryHackMe-212C42?style=for-the-badge&logo=tryhackme&logoColor=white"/></a>
  <a href="https://www.youtube.com/@itzmunazah"><img src="https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white"/></a>
  <a href="https://munazajameel.site/blog"><img src="https://img.shields.io/badge/Blog-000000?style=for-the-badge&logo=hashnode&logoColor=white"/></a>
</p>
---
👩‍💻 About Me
🎓 Completed a 6-month Cybersecurity Diploma and self-learning towards the SOC domain
🏠 Built a home SOC lab using VirtualBox with Kali Linux, Windows, and Ubuntu Server
🔍 Focused on threat detection, SIEM, log analysis, and incident response
📖 Currently studying for CompTIA Security+ and completing TryHackMe SOC Level 1
🌍 Based in Pakistan | Open to Junior SOC Analyst / Security Analyst roles
---
🛡️ SOC & Blue Team Skills
Category	Tools & Technologies
SIEM	Wazuh, Splunk (Free Tier)
Log Analysis	Windows Event Viewer, Sysmon, Wazuh Alerts
Network Analysis	Wireshark, Nmap, Netdiscover
Threat Simulation	Kali Linux, Hydra, Metasploit, CrackMapExec
Operating Systems	Windows 10/11, Ubuntu Server, Kali Linux
Frameworks	MITRE ATT&CK (awareness)
---
🔬 Featured Projects
🔵 Blue Team (SOC)
Project	Description	Tools
SOC SMB BruteForce Detection	Simulated SMB brute force attack, detected via SIEM alerts, wrote incident report	Wazuh, Kali, Windows, CrackMapExec
> *More SOC projects coming soon — actively building the lab 🚧*
🔴 Red Team / Ethical Hacking (Learning Phase)
Project	Description	Tools
Ethical Hacking Lab	Full pentest workflow: recon → exploit → post-exploit → report	Kali, Metasploit, Nmap
Scanning Phase Practice	Port scanning, host discovery, vulnerability scanning	Nmap, Nikto, Zaproxy, Netdiscover
---
📈 My Learning Path
```
[✅] Cybersecurity Diploma (6 months)
[✅] Home Lab Setup — VirtualBox + Kali + Windows
[✅] Ethical Hacking Fundamentals (all phases)
[✅] First SOC Project — SMB Brute Force Detection with Wazuh
[🔄] TryHackMe SOC Level 1 Path
[🔄] Wazuh SIEM Lab — expanding scenarios
[⬜] CompTIA Security+
[⬜] Blue Team Labs Online — 3 investigations
[⬜] BTL1 Certification
```
---
📊 GitHub Stats
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=munazajamil&show_icons=true&theme=github_dark&hide_border=true&count_private=true" height="150"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=munazajamil&theme=github-dark-blue&hide_border=true" height="150"/>
</p>
---
📬 Connect With Me
I'm actively building my SOC skills and looking for opportunities. Feel free to reach out!
<p align="center">
  <a href="https://www.linkedin.com/in/munaza-jamil1/">LinkedIn</a> •
  <a href="https://tryhackme.com/p/munaza.jamil01">TryHackMe</a> •
  <a href="https://munazajameel.site/blog">Blog</a>
</p>
---
<p align="center"><i>"Every alert is a story. My job is to read it."</i></p>
